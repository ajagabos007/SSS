...
<div class="panel panel-primary col-sm-offset-1 col-sm-10 login-form">
    <div class="panel panel-heading">
     ...
     <div class="panel panel-body">
            <form method="POST" action="{{route('admin.login.submit')}}"  aria-label="{{ __('Login') }}" class="form-horizontal" role="form">
            @csrf
                <div class="form-group">
                    <label class="control-label col-sm-3" for="email">{{__('E-Mail Address')}}</label>
                    <div class="col-sm-9">
                    <input type="email" class="form-control {{ $errors->has('email') ? ' is-invalid' : '' }}" id="email" name="email" value="{{ old('email') }}" placeholder="Enter email" required autofocus>
                       @if ($errors->has('email'))
                ...
                <div class="form-group ">
                    <label class="control-label col-sm-3" for="password">
                        {{__('Password')}} <span class="glyphicon glyphicon-lock"></span>
                    </label>
                    <div class="col-sm-9 ">
